...
export async function ingestParsedAsUser(parsed: ParsedPayload) {
  ...
  if (parsed.profile) {
    ...
    for (const key of profileFields) {
      ...
      if (Object.prototype.hasOwnProperty.call(parsed.profile, key)) {
        safeProfile[key] = (parsed.profile as Record<string, unknown>)[key];
      }
    }

    // Defensive fallback: if safeProfile is empty or missing all required fields, set all required defaults
    let hasAnyRequired = requiredFields.some(f => safeProfile[f] && String(safeProfile[f]).trim() !== '');
    if (!hasAnyRequired) {
    ...
    const hasRequired = requiredFields.some(f => safeProfile[f] && String(safeProfile[f]).trim() !== '');
    ...
    if (Object.keys(safeProfile).length === 0 || !requiredFields.some(f => safeProfile[f] && String(safeProfile[f]).trim() !== '')) {
    ...
    if (!didUpdate && (Object.keys(safeProfile).length === 0 || !requiredFields.some(f => safeProfile[f] && String(safeProfile[f]).trim() !== ''))) {
    ...
    if (updErr || !didUpdate) {
      const { error: insErr, data: insData } = await supabaseClient.from('profiles').insert(safeProfile);
      if (insErr) {
        console.error('[Profile insert failed] safeProfile:', safeProfile, 'Error:', insErr, 'InsertData:', insData);
        throw new Error('Profile insert failed: ' + (insErr.message || JSON.stringify(insErr)));
      } else {
        console.log('[Profile insert succeeded] safeProfile:', safeProfile, 'InsertData:', insData);
      }
    }
  }
  ...
  if (Array.isArray(parsed.experiences)) {
    ...
    const safeExperiences = parsed.experiences.map((it, idx: number) => ({
      ...
      order_index: idx,
      title: (it as Record<string, unknown>).title || (it as Record<string, unknown>).job_title || null,
      company: (it as Record<string, unknown>).company || (it as Record<string, unknown>).employer || null,
      start_date: (it as Record<string, unknown>).start_date || (it as Record<string, unknown>).start_year || null,
      end_date: (it as Record<string, unknown>).end_date || (it as Record<string, unknown>).end_year || null,
      is_current: !!(it as Record<string, unknown>).is_current,
      description: (it as Record<string, unknown>).description || null,
    }));
  ...
  if (Array.isArray(parsed.education)) {
    ...
    const safeEducation = parsed.education.map((it) => ({
      profile_id: profileId,
      school: (it as Record<string, unknown>).school || (it as Record<string, unknown>).institution || null,
      degree: (it as Record<string, unknown>).degree || (it as Record<string, unknown>).qualification || null,
      start_year: (it as Record<string, unknown>).start_year || null,
      end_year: (it as Record<string, unknown>).end_year || null,
      description: (it as Record<string, unknown>).description || null,
    }));
  ...
  if (Array.isArray(parsed.skills)) {
    ...
    const safeSkills = parsed.skills.map((it) => ({
      profile_id: profileId,
      skill: typeof it === 'string' ? it : (it as Record<string, unknown>).skill || JSON.stringify(it)
    }));
